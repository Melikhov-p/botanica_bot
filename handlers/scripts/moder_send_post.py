...
async def moders_command_send_post(callback: types.CallbackQuery, state: FSMContext):
    keyboard = {'post_photo': 'Добавить фото', 'post_title': 'Добавить заголовок', 'post_description': 'Добавить описание', 'post_price': 'Добавить цену', 'post_preview': 'Предпросмотр'}
    ...
    await bot.send_message(callback.from_user.id, f"📋 Создание поста\n\n"
                                                  ...
                                                  f"Для отмены создания поста /cancel", reply_markup=keyboard_generator(keyboard), parse_mode='html')
...
async def moders_callback_post_preview(callback: types.CallbackQuery, state: FSMContext):
    ...
    if post_data.get('photo'):
        ...
        await moders_command_send_post(callback, state)
